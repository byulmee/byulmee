...
<body>
	...
	<div class="outer">
	<form action="pupdate.wr" method="post" enctype="Multipart/form-data">
		...
		<div class="mainInput">
			<div class="radiogroup">
				<input type="radio" class="checkbox" name="productGroup" value="activity" id="Group1">
				...
				<input type="hidden" name="proCategory" id="proCategory" value="${ product.proCategory }">
			</div>
			<script>
				$(document).ready(function(){
					var setCategory = $('#proCategory').val();
					if(setCategory == "0"){
			...
			<div class="inputGroup">
				...
				<div class="inputText">
					제목&nbsp;&nbsp;<input type="text" name="proTitle" class="inputValue" id="inputTitle" value="${ product.proTitle }"><br> 
					상품 가격&nbsp;&nbsp;<input type="number" name="proPrice" class="inputValue" id="inputTitle" value="${ product.proPrice }"><br> 
					배송비&nbsp;&nbsp;<input type="number" name="proCharge" class="inputValue" id="inputTitle" value="${ product.proCharge }"><br>
					상품 재고&nbsp;&nbsp;<input type="number" name="proStock" class="inputValue" id="inputTitle" value="${ product.proStock }"><br>
					최대 구매 가능 수량&nbsp;&nbsp;<input type="number" name="proMaximum" class="inputValue" id="inputTitle" value="${ product.proMaximum }"><br> 
					관련 활동 링크&nbsp;&nbsp;<input type="text" name="proUrl" class="inputValue" id="inputTitle" placeholder="http://" value="${ product.proUrl }">
				</div>
			</div>
		</div>
		<div class="InputName">
		...
		<div class="TextArea">
			<textarea type="text" name="proContent" class="form-control" id="inputContent" style="height: 300px">${ product.proContent }</textarea>
		</div>
		<div class="InputName">
			<p class="inputName">구매 유의 사항 및 환불 규정</p>
		</div>
		<div class="TextArea">
			<textarea type="text" name="proGuide" class="form-control" id="inputContent" style="height: 150px">${ product.proGuide }</textarea>
		</div>
		<div id="buttonArea">
			<button type="button" id="cancelBtn" class="btn btn mb-3">삭제</button>
			<button type="button" id="unableBtn" class="btn btn mb-3" onclick="location.href='prounable.wr?proNo=${ product.proNo }&page=${ page }'" style="<c:if test="${ product.proStatus eq 'Y' }">background: #ECECEC; color: #585858</c:if>
					<c:if test="${ product.proStatus eq 'N' }">background: #FF6833; color: white</c:if>">비활성</button>
			<button type="button" id="ableBtn" class="btn btn mb-3" onclick="location.href='proable.wr?proNo=${ product.proNo }&page=${ page }'" style="<c:if test="${ product.proStatus eq 'Y' }">background: #FF6833; color: white</c:if>
					<c:if test="${ product.proStatus eq 'N' }">background: #ECECEC; color: #585858</c:if>">활성</button>
	...
	<script>
		$(function (){
			$('#cancelBtn').click(function(){
				...
				if(chk) {
					location.href="delproduct.wr?proNo=${ product.proNo }&page=${ page }" + "&imgName1=" + imgName1+ "&imgName2=" + imgName2+ "&imgName3=" + imgName3 + "&imgName4=" + imgName4+ "&imgName5=" + imgName5;
				}
			});
		});
	</script>
