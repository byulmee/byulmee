...
<body>
	...
	<div class="outer">
		<div class="MainLogoText">
			<p class="MainText">고객 Q&A</p>
		</div>
		<div class="table table-responsive" id="TableArea">
			<table class="table">
				<tr>
					<th class="category">분류 > <c:choose>
								<c:when test="${ salesqna.salqnaType eq 'A' }">활동문의</c:when>
								<c:when test="${ salesqna.salqnaType eq 'P' }">상품문의</c:when>
							</c:choose></th>
				</tr>
				<tr>
					<th class="mainTitle">${ salesqna.salqnaTitle }</th>
				</tr>
				<tr>
					<th class="secondTitle">${ salesqna.salqnaDate }<br>${ salesqna.member.memNickname } 작성</th>
				</tr>
				<tr class="textArea">
					<% pageContext.setAttribute("newLineChar", "\r\n"); %>
					<td>${ fn:replace(salesqna.salqnaContent, newLineChar, "<br>") }</td>
				</tr>
			</table>
			<c:if test="${ empty reply }">
			<div class="answerArea">
				<c:if test="${ loginUser.memId eq salesqna.salqnaStarcode }">
		...
		<div class="buttonArea">
			<c:if test="${ loginUser.memId eq salesqna.memId }">
				<button type="button" id="delBtn" class="btn btn mb-3" style="background-color: #EFEFEF; color: #5A5A5A">삭제</button>
				<button type="button" id="updateBtn" class="btn btn mb-3" style="background-color: #FF6833; color: white" onclick="location.href='salQnaUpdateForm.wr?salqnaNo=${ salesqna.salqnaNo }&page=${ page }'">수정</button>
			</c:if>
			<button type="button" id="listBtn" class="btn btn mb-3" style="background-color: #FF6833; color: white" onclick="location.href='customerView.wr?page=${ page }'">목록으로</button>
		</div>
	</div>
	<script>
		...
		$('.modalDelBtn').on("click", function(){
			...
			if(chk){
				$.ajax({
					url: 'delSalReply.wr',
					data: {repNo:repNo, repRefno:repRefno},
					success: function(data){
						alert("답글 삭제 완료");
						$("#modifyModal").modal("hide");
						window.location.reload();
		...
		$(function(){
			$('#delBtn').click(function(){
				var chk = confirm("정말 삭제하시겠습니까?");
				if(chk){
					location.href="deleteSalQna.wr?salqnaNo=${ salesqna.salqnaNo }&page=${ page }";
				}
			});
		});
	</script>
